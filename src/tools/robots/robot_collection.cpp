...
void RobotCollection::Awake(std::string account, int min, int max)
{
    ...
    else
    {
        ...
        {
            CreateRobot(account + std::to_string(i));
        }
    }
...
void RobotCollection::HandleNetworkConnected(Robot* pRobot, Packet* pPacket)
{
    ...
    // locator
    auto pRobotLocator = ComponentHelp::GetGlobalEntitySystem()->GetComponent<RobotLocator>();
    pRobotLocator->RegisterToLocator(pRobot);
}
...
void RobotCollection::HandleGameToken(Robot* pRobot, Packet* pPacket)
{
    ...
    auto pTokenObj = pRobot->GetComponent<RobotComponentGameToken>();
    if (pTokenObj == nullptr)
        pRobot->AddComponent<RobotComponentGameToken>(proto.token().c_str(), proto.ip().c_str(), proto.port());
    else
        pTokenObj->Update(proto.token().c_str(), proto.ip().c_str(), proto.port());
